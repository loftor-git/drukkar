...
$form_post = array('password', 'title', 'date', 'date_backup', 'tags', 'format', 'files', 'translit', 'submit', 'text');
$form_get = array('file');
...
if (isset($_SESSION['is_logged_in'])) {
    ...
    if (!$file_to_edit) {
        ...
        echo "<p><a href=\"$me?file=" . date($blog_file_name_format) . ".xml\">$loc_new</a></p>";
        foreach (array_reverse(glob($blog_entries_dir . "*.xml")) as $file) {
            $entry = entry_load($file);
            echo "<a href=\"$me?file=" . basename($file) . "\"> " . date($blog_date_format, (int) $entry->date) . " " . ((string) $entry->format === "html" ? $entry->title : htmlspecialchars($entry->title)) . "</a><br>";
       }
    } else {
        $entry_exists = file_exists($blog_entries_dir . $file_to_edit);
        echo "<form name=\"form\" action=\"$me?file=" . $file_to_edit . "\" method=\"post\" enctype=\"multipart/form-data\">";
        ...
        if ($entry_exists) {
            echo "<p><a href=\"" . $blog_base_location . "index.php?post=" . basename($file_to_edit, ".xml") . "\" target=\"_blank\">$loc_view_entry</a></p>";
        }
...
#        var_dump($entry->old_date);
            ...
            echo "<p>$loc_date<br><input type=\"text\" size=20 name=\"date\" value=\"" . date($blog_date_format, ($new_entry ? $time : (int) $entry->date)) . "\"><input type=\"hidden\" name=\"date_backup\" value=\"" . date($blog_date_format, ($new_entry ? $time : (int) $entry->date)) . "\"></p>";
        }
        
        echo "<p>$loc_upload<br><input type=\"file\" name=\"file1\"><br><input type=\"file\" name=\"file2\"><br><input type=\"file\" name=\"file3\"></p>
        ...
        <p><input type=\"submit\" name=\"submit\" value=\"$loc_save\"> <input type=\"submit\" name=\"submit\" value=\"$loc_delete\" onClick=\"javascript:return confirm('$loc_delete_prompt_entry');\"></p>
        </form>";
        
        }
} else {
    echo "<form name=\"form\" action=\"$me\" method=\"post\" enctype=\"multipart/form-data\">";
    echo "<p>$loc_password<br><input type=\"password\" name=\"password\" value=\"\"></p>";
    echo "<p><input type=\"submit\" name=\"submit\" value=\"$loc_log_in\"></p></form>";
}
